...
const KillFeed = ({ killFeedData = [], currentTime = 0, isReplayMode = false }) => {
  ...
  const lastKillTime = useRef({}); // Track last kill times for each attacker

  useEffect(() => {
    if (isReplayMode) {
      // In replay mode, filter events based on current time
      const currentEvents = killFeedData
      ...
      ws.onmessage = (message) => {
        ...
        setFeed((prev) => [formatted, ...prev].slice(0, 10));

        // Update last kill time
        lastKillTime.current[data.attacker_id] = timestamp;
      };
