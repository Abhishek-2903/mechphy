...
const KillFeed = () => {
  ...
  const lastKillTime = useRef({}); // Use useRef to avoid dependency issues
  ...
  useEffect(() => {
    ...
    ws.onmessage = (message) => {
      ...
      if (lastKillTime.current[attackerId]) {
        const timeDifference = formatted.timestamp - lastKillTime.current[attackerId];
      ...
      lastKillTime.current[attackerId] = formatted.timestamp;
  ...
  // Display multi-kill counts (this removes the unused variable warning)
  const getMultiKillDisplay = (attackerId) => {
    const count = multiKillCounts[attackerId];
    if (count && count > 1) {
      return ` (${count + 1} kills!)`;
    }
    return "";
  };
      ...
      {feed.map((entry) => (
        <div key={entry.id} className="kill-entry">
          <p>
            ...
            from {entry.distance} km{getMultiKillDisplay(entry.attackerId)}
          </p>
        </div>
      ))}
    </div>
